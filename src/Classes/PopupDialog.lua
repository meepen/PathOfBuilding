...
function PopupDialogClass:Draw(viewPort)
	...
	local titleWidth = graphics:DrawStringWidth(16, "VAR", title)
	local titleX = x + m_floor((width - titleWidth - 8) / 2)
	graphics:SetDrawColor(1, 1, 1)
	graphics:DrawImage(nil, titleX, y - 10, titleWidth + 8, 24)
	graphics:SetDrawColor(0, 0, 0)
	graphics:DrawImage(nil, titleX + 2, y - 8, titleWidth + 4, 20)
	graphics:SetDrawColor(1, 1, 1)
	graphics:DrawString(titleX + 4, y - 7, "LEFT", 16, "VAR", title)
	-- Draw controls
